import 'dart:io' as io;

import 'package:injectable/injectable.dart';
import 'package:racs_reader/core/constants/env.dart';
import 'package:racs_reader/core/storage/secure_storage.dart';
import 'package:racs_reader/core/storage/settings_storage.dart';

@lazySingleton
class HttpClient {
  /// Maximum time to wait both for the connection and for the server to start
  /// responding. Kept short so a slow or dead server can't freeze a scan — the
  /// caller treats a timeout like an unreachable server (queues to the DLQ).
  static const _timeout = Duration(seconds: 5);

  final SettingsStorage _settingsStorage;
  final SecureStorage _secureStorage;

  String? _host;

  HttpClient(this._settingsStorage, this._secureStorage);

  Future<String> get host async {
    _host ??= await _settingsStorage.getHost();
    return _host!;
  }

  void resetHost() => _host = null;

  Uri _uri(String host, String path, [Map<String, dynamic>? queryParameters]) =>
      Env.apiSecure
      ? Uri.https(host, path, queryParameters)
      : Uri.http(host, path, queryParameters);

  Future<void> _attachAuth(io.HttpClientRequest request) async {
    request.headers.set(io.HttpHeaders.acceptHeader, 'application/json');
    final token = await _secureStorage.getAccessToken();
    if (token != null) {
      request.headers.set(io.HttpHeaders.authorizationHeader, 'Bearer $token');
    }
  }

  Future<io.HttpClientResponse> get(
    String path, {
    Map<String, dynamic>? queryParameters,
  }) async {
    final h = await host;
    final uri = _uri(h, path, queryParameters);
    final client = io.HttpClient()..connectionTimeout = _timeout;
    final request = await client.getUrl(uri);
    await _attachAuth(request);
    return request.close().timeout(_timeout);
  }

  Future<io.HttpClientResponse> post(String path, {Object? body}) async {
    final h = await host;
    final uri = _uri(h, path);
    final client = io.HttpClient()..connectionTimeout = _timeout;
    final request = await client.postUrl(uri);
    await _attachAuth(request);
    if (body != null) {
      request.headers.contentType = io.ContentType.json;
      request.write(body);
    }
    return request.close().timeout(_timeout);
  }

  Future<io.HttpClientResponse> put(String path, {Object? body}) async {
    final h = await host;
    final uri = _uri(h, path);
    final client = io.HttpClient()..connectionTimeout = _timeout;
    final request = await client.putUrl(uri);
    await _attachAuth(request);
    if (body != null) {
      request.headers.contentType = io.ContentType.json;
      request.write(body);
    }
    return request.close().timeout(_timeout);
  }

  Future<io.HttpClientResponse> delete(String path) async {
    final h = await host;
    final uri = _uri(h, path);
    final client = io.HttpClient()..connectionTimeout = _timeout;
    final request = await client.deleteUrl(uri);
    await _attachAuth(request);
    return request.close().timeout(_timeout);
  }
}
