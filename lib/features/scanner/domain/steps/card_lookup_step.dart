...
class CardLookupStep extends ScanStep {
  ...
  Future<ScanContext> execute(ScanContext context) async {
    await logger.debug('Looking up card with value: "${context.rawValue}"', className: 'CardLookupStep');
    final card = await _localRepository.getCardByValue(context.rawValue);
    if (card == null) {
      await logger.warning('Card not found for value: "${context.rawValue}"', className: 'CardLookupStep');
      throw ScanException('Card not found', context, flag: ScanFlag.unknownCard);
    }
