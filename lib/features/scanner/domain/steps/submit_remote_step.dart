...
class SubmitRemoteStep extends ScanStep {
  ...
  Future<ScanContext> execute(ScanContext context) async {
    ...
    // From here on this step owns remote handling for the scan whatever the
    // server answers, so mark it submitted so the caller never re-sends it.
    context = context.copyWith(submitted: true);
    await logger.debug(
