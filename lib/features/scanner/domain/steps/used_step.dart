...
class UsedStep extends ScanStep {
  UsedStep(super.logger);

  @override
  Future<ScanContext> execute(ScanContext context) async {
    final card = context.card!;
    await logger.debug(
      'Checking used status for card: ${card.id}',
      className: 'UsedStep',
    );

    if (card.used) {
      await logger.warning(
        'Card already used: ${card.id} at ${card.usedAt}',
        className: 'UsedStep',
      );
      throw ScanException(
        'Card has already been used',
        context,
        flag: ScanFlag.duplicateAttemptMesh,
      );
    }

    await logger.info(
      'Used check passed for card: ${card.id}',
      className: 'UsedStep',
    );
    return context;
  }
