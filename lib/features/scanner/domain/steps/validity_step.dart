...
class ValidityStep extends ScanStep {
  ...
  Future<ScanContext> execute(ScanContext context) async {
    ...
    await logger.debug(
      'Checking validity for card: ${card.id}',
      className: 'ValidityStep',
    );

    if (card.validFrom != null && now.isBefore(card.validFrom!)) {
      await logger.warning(
        'Card not yet valid: ${card.id}',
        className: 'ValidityStep',
      );
      throw ScanException(
    ...
    await logger.info(
      'Validity check passed for card: ${card.id}',
      className: 'ValidityStep',
    );
    return context;
  }
