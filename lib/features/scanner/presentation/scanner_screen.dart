...
import 'package:racs_reader/core/constants/env.dart';
import 'package:racs_reader/common/widgets/app_button.dart';
import 'package:racs_reader/common/widgets/glass_card.dart';
import 'package:racs_reader/features/cards/domain/card.dart' as card_domain;
...
class _ScannerScreenState extends State<ScannerScreen>
  ...
  // Dev-only: tapping the scan box injects this instead of a real QR code.
  static const _devFakeQr = '123';
  ...
  void initState() {
    ...
    _controller = MobileScannerController(
      ...
      // The view model already de-dupes: onScan() flips state out of idle
      // synchronously and _onDetect ignores every frame until reset(). Use
      // `normal` so re-presenting the same code (or the same one after "Scan
      // Again") is detected on the next frame instead of requiring the code to
      // leave and re-enter the box, which is what `noDuplicates` demands.
        ...
        builder: (context, constraints) {
          ...
          return Stack(
            ...
            children: [
              ...
              // Dev-only: tap the scan box to inject a fake QR code. Placed
              // above the decorative layers so nothing swallows the tap, but
              // below the result modal (which only shows when state != idle).
              if (Env.isDev)
                Positioned.fromRect(
                  rect: boxRect,
                  child: GestureDetector(
                    ...
                    onTap: () {
                      if (_viewModel.state == ScanState.idle) {
                        _viewModel.onScan(_devFakeQr);
                      }
                    },
                  ),
                ),
