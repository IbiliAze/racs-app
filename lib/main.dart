...
import 'package:racs_reader/core/router/app_router.dart';
...
void main() async {
  ...
  if (isConnected) {
    ...
    try {
      ...
      await getIt<AuthService>().getAuthReader();
      ...
      // Cards are scoped to the campaign picked in Settings; nothing to
      // download until one has been selected.
      final campaignId = await getIt<SettingsService>().getCampaignId();
      if (campaignId != null) {
        unawaited(getIt<CardService>().downloadCards(campaignId));
      }
    } catch (_) {
